...
fn open_write_store(path: &Path) -> Result<FilesystemWalStore, String> {
    ...
    store
        .acquire_writer_epoch(WriterEpochRequest {
            epoch_id: epoch_id(),
            storage_fencing_token: digest("hello-effect-patch:fencing"),
            process_identity: digest("hello-effect-patch:process"),
            host_identity: digest("hello-effect-patch:host"),
            started_at_lsn: Lsn::from_raw(0),
            previous_epoch_id: None,
            previous_epoch_final_commit_digest: None,
            lease_or_lock_evidence: digest("hello-effect-patch:lease"),
        })
...
fn epoch_id() -> WriterEpochId {
    WriterEpochId::from_hash(digest("hello-effect-patch:epoch"))
}
